# データベース設計書 (Schema Definition)

**Version:** 6.3 (Rich Vocabulary Features)
**Target:** Supabase (PostgreSQL 15+)

## 1\. ER図 (Entity Relationship)

`vocabulary` テーブルに、言語学習を深めるためのメタデータフィールド（IPA、語源、類義語など）を追加しました。

```mermaid
erDiagram
    users ||--|| profiles : "1:1 User Settings"
    users ||--o{ categories : "Owns"
    users ||--o{ vocabulary : "Owns"
    categories ||--o{ vocabulary : "Categorizes"
    ...
    profiles {
        ...
        text encrypted_api_key "E2EE Storage"
        boolean is_ai_auto_complete_on
    }

    categories {
        uuid id PK
        uuid user_id FK
        text name "ex. 'Gold Phrase'"
    }
    ...
    vocabulary {
        ...
        text term "Target Word"
        text definition "Meaning"
        text part_of_speech "New: PoS"
        text example "Context Sentence"
        
        text ipa "New: Pronunciation"
        text etymology "New: Origin"
        text[] synonyms "New: Similar words"
        text[] collocations "New: Word pairs"
        text source_memo
        
        vector embedding "1536 dim"
        boolean is_generating
        enum status "SRS Status"
    }
```

## 2\. SQL Schema (DDL)

### 2.1. 初期設定 (Extensions & Enums)
...
更新: 戻り値に `part_of_speech` を追加しました。クイズ画面で「名詞が正解なのに動詞が選択肢に出る」などの違和感を防ぐフィルタリングに利用できます。
